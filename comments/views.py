...
class CommentDetail(APIView):
    def get_object(self, pk):  # selecting a comment object by the pk id
        try:
            return Comment.objects.get(pk=pk)  # here we returning the comment by the id
        except Comment.DoesNotExist:  # if the exception 'comment.does.not.exist' occurs, ...
    ...
    def get(self, request, pk):  # 'get' is getting something fom the database.. in this case a single song
        ...
        return Response(serializer.data,
                        status=status.HTTP_200_OK)  # here we are returning the selected song, in the serialized/updated format
    ...
    def put(self, request, pk):  # 'put' syntax == update functionality
        ...
        serializer = CommentSerializer(comment,
                                       data=request.data)  # here we are reformatting the song using the serializer
        if serializer.is_valid():  # == if the data in the request matches the model ...
            serializer.save()
            return Response(serializer.data,
                            status=status.HTTP_200_OK)  # save the selected song with the new data from the request
